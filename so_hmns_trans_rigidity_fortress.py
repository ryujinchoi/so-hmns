...
class SoHmnsTransRigidityFortress:
    ...
    def __init__(self):
        ...
        self.M_ultimate_fortress = np.zeros((self.dims, self.dims), dtype=object)
    ...
    def _freeze_immutable_ultimate_metric(self):
        ...
        for i in range(self.dims):
            for j in range(self.dims):
                self.M_ultimate_fortress[i, j] = Fraction(1, 1) if i == j else Fraction(0, 1)
    ...
    def verify_absolute_fortress(self) -> bool:
        ...
        float_rep = np.array([[float(cell) for cell in row] for row in self.M_ultimate_fortress])
...
if __name__ == "__main__":
    fortress = SoHmnsTransRigidityFortress()
