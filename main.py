...
def fetch_live_usgs_data():
    """USGS 글로벌 지진 관측소로부터 최신 데이터 실시간 동기화"""
    ...
    try:
        ...
        print(f"⚠️ 실시간 API 지연 발생 (기본 내장 데이터셋으로 대체 연산): {e}")
        return []
...
def log_to_target_file(log_text):
    """[보완] target_hmns.txt 파일에 예보 결과 기록을 실시간 자동 누적"""
...
def run_combined_forecast_system():
    ...
    print("="*75)
    print(" [최종 통합 종결판] 신뢰구간 산출 및 로그 자동 저장 예보 엔진 ")
    print("="*75)
    ...
    start_log = f"\n=== 예보 세션 시작: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===\n▶ 분석 대상: {target_name}"
    ...
    while t <= t_end:
        ...
        # [보완] 비선형 시스템의 노이즈를 반영한 앞뒤 오차 범위 산출 (±3일 수준 스케일링)
        error_range = timedelta(days=3 * cfg["scale_factor"] / 4.0)
        min_window = (event_time - error_range).strftime("%m/%d")
        max_window = (event_time + error_range).strftime("%m/%d")
        window_str = f"(오차범위: {min_window}~{max_window})"
        ...
        if t >= t_mainshock and not tsunami_triggered:
            msg = f"🚨 [{current_timestamp_str} {window_str}] {target_name} 단층 임계 돌파 본진! [규모: M {cfg['max_magnitude']:.1f}]"
        ...
        if W > c_safe and W <= c_rupture and not volcano_alert:
            msg = f" can🌋 [{current_timestamp_str} {window_str}] 복합 마그마 에너지 위험 임계 경계 진입."
        ...
        if int(t * 5) % 4 == 0:
            print(f"[{current_timestamp_str}] 규모: M {current_magnitude:.1f} | 포함에너지(W): {W:4.2f} | 🌊쓰나미파고: {tsunami_height:5.2f}m")
            
        if tsunami_height > 12.0 and cfg["tsunami_active"]:
            msg = f"🌊🌊 [{current_timestamp_str} 최종대피] 쓰나미 해안 습격 분출 도달 완료."
            print("\n" + msg); log_to_target_file(msg)
            break
        t += dt
    
    end_log = f"=== 예보 세션 종료: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} ===\n"
    log_to_target_file(end_log)
    print("="*75)
    print("💡 분석 결과가 'target_hmns.txt' 파일에 안전하게 자동 기록되었습니다.")
    print("="*75)
