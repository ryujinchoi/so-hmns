...
def verify_so_hmns_dark_matter():
    ...
    # 1. 기호 변수 정의 (부동 소수점 오차 원천 배제)
    r = sp.Symbol('r', positive=True)         # 은하 중심으로부터의 격자 주소 거리
    G = sp.Symbol('G', positive=True)         # 중력 가중치 상수
    W_core = sp.Symbol('W_core', positive=True) # 은하 중심부 물질 가중치 (질량)
    a_0 = sp.Symbol('a_0', positive=True)     # SO-HMNS 최소 연산 임계 가속도 한계값
    ...
    # 6. 전역 닫힘성 제약식 (Kernel Validation) 판정 루틴
